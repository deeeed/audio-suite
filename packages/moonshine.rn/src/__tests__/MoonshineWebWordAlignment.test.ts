...
describe('alignWordsFromCrossAttention', () => {
  ...
  it('keeps consecutive subword tokens in the same aligned word', () => {
    const vocabById: string[] = [];
    vocabById[1] = '<s>';
    vocabById[2] = '</s>';
    vocabById[10] = '▁Hello';
    ...
    const crossAttention = new Float32Array([
      12, 12, 12, 8, 2, 1, 1, 1, 1, 2, 8, 12, 12, 12, 12, 12,
    ]);
    ...
    expect(words[0]?.word).toBe('Helloworld');
    expect(words[0]?.startTimeMs).toBe(0);
    expect(words[0]?.endTimeMs).toBeGreaterThan(words[0]?.startTimeMs ?? 0);
  });

  it('returns no words when the BOS/EOS slice has no text tokens', () => {
    const tokenizer = createFakeTokenizer();
