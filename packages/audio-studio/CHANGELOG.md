...
- Android `trimAudio` and iOS `startRecording` now reject a filename that is not a single path component. Both appended the caller's value to a directory, so `../../../../tmp/pwned` wrote outside it — `File(filesDir, ...)` and `appendingPathComponent` resolve `..` alike. Android rejects with `INVALID_OUTPUT_FILENAME`, iOS with an invalid-settings failure (#452).
