...
- Android device changes now claim their recording session before the transition delay and recheck it before restart. A concurrent stop could previously finish during that delay, leaving a header-only WAV or restarting an `AudioRecord` after the manager had stopped (#472).
