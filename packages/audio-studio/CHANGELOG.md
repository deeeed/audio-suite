...
- Android device changes now hold the recorder lock through release, transition, and restart. A concurrent stop could previously finish during the 200 ms transition delay, leaving a header-only WAV or restarting an `AudioRecord` after the manager had stopped (#472).
